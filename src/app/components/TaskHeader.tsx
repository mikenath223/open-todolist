...
export default function TaskHeader({ selectedGroupItem }: $Props) {
  ...
  return (
    <header className="my-8 flex h-12 w-full items-center justify-between">
      <div className="flex items-center gap-4">
        ...
        {selectedGroupItem.id !== '0' && (
          <Button variant="contained" onClick={() => setOpenAddTodo(true)}>
            Add Task
          </Button>
        )}
      </div>
      <AddTodo
        ...
        isOpen={openAddTodo}
        closeModal={() => setOpenAddTodo(false)}
      />
    </header>
  );
}
